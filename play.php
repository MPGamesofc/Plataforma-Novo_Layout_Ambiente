  ...
  */
  ...
  if(isset($_GET["slug"], $_GET["id"]) && $secure->isNumber($_GET["id"])){
   	...
   	if($user["position"] == 1 OR $user["position"] == 2){
   		$find = $db->query("SELECT name, mobile, plays, MesDosJogos, DataJogos FROM games WHERE id = $gid LIMIT 1"); 
   	} else {
   		$find = $db->query("SELECT name, mobile, plays, MesDosJogos, DataJogos FROM games WHERE status = 1 AND id = $gid LIMIT 1");   	   		
   	}
   	...
   	if($check->isMobile()){
   			if($find->num_rows < 1){
   				...
   				$datadebug = $game["DataJogos"];
   	
   	            if($datadebug != date("n")){
   	                $db->query("UPDATE games SET MesDosJogos = 0 WHERE id = $gid LIMIT 1");
            	}
   		
   				$new2 = $game["MesDosJogos"] +1;
   	
   				$db->query("UPDATE games SET plays = $new WHERE id = $gid LIMIT 1");
   	
   	            $data = date("m");
   				$db->query("UPDATE games SET MesDosJogos = $new2 WHERE id = $gid LIMIT 1");
   	            $db->query("UPDATE games SET DataJogos = $data WHERE id = $gid LIMIT 1");
